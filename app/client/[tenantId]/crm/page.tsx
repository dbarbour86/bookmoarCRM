...
import { Users, Plus, Tag, Phone, Mail, Car, AlertTriangle, Download, FileSpreadsheet, X, RefreshCw, Loader2 } from 'lucide-react';
...
export default function ClientCRMPage() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/crm/contacts?tenantId=${tenantId}`);
      if (res.ok) {
        const data = await res.json();
        if (data.contacts) {
      ...
      console.warn('[CRM_FETCH_ERR] Falling back to local state:', err);
    } finally {
    ...
    <div className="space-y-6">
      ...
      {isLoadingContacts ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-500 text-xs flex flex-col items-center justify-center gap-2">
          ...
          <span>Loading persistent CRM contacts...</span>
        </div>
      ) : contacts.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-500 text-xs">
          No contacts found in CRM. Submit a quote request from the client website to add a lead!
        </div>
      ) : (
